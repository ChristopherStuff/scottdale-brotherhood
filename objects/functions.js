...
exports.createGroup = async (server, groups, serverid, name, level_name) => {
    return new Promise((resolve, reject) => {
        ...
        server.query(`INSERT INTO \`groups\` (\`server\`, \`name\`, \`levels\`) VALUES ('${serverid}', '${name}', '${JSON.stringify([{"level":1,"name":"Создатель группы","manage_group":true,"manage_levels":[1]}])}')`, (error) => {
            if (error){
